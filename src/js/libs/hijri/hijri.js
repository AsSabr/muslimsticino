...
var HijriJS = {
	...
	hijriToGregorian: function hijriToGregorian( year, month, day ) {
		...
		const i = iln - 16260;
		const mcjdn = id + HijriJS.ummalqura_dat[i - 1] - 1;
	...
	gregorianToHijri: function gregorianToHijri( pYear, pMonth, pDay ) {
		...
		for ( var i = 0; i < HijriJS.ummalqura_dat.length; i++ ) {
			if ( HijriJS.ummalqura_dat[i] > mcjdn ) {break;}
		}
		...
		const iy = ii + 1;
		const im = iln - 12 * ii;
		const id = mcjdn - HijriJS.ummalqura_dat[i - 1] + 1;
		const ml = HijriJS.ummalqura_dat[i] - HijriJS.ummalqura_dat[i - 1];
	...
	HDate: function HDate( year, month, day ) {
		...
		this.format = function useFormat( year, month, day, format ) {
			if ( HijriJS.vlidateHijri( year, month, day ) ) {
				...
				{newFormat = newFormat.replace( "dd", "10" > day ? "0" + day : day );}
				} else {newFormat = newFormat.replace('d', day);}

				if ( newFormat.indexOf( 'mm' ) !== -1 ) {
				{newFormat = newFormat.replace( "mm", "10" > month ? "0" + month : month );}
				...
				newFormat = newFormat.replace(
					'N',
					HijriJS.lang[HijriJS.currentLanguage].notation
				);
				return HijriJS.lang[HijriJS.currentLanguage].formatLocale(
					newFormat
				);
			}
		};
