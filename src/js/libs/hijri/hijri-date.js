...
function HijriDate(year, month, date, hour, minute, second, millisecond) {
  ...
  this.getFullYearString = function () {
    return 0 < year
      ? HijriDate.toDigit(year, 4) + " H"
      : HijriDate.toDigit(Math.abs(year - 1), 4) + " BH";
  };
  ...
  function updateDate() {
    ...
    month = 0 <= month ? month % 12 : 0 === month % 12 ? 0 : 12 + (month % 12);
   ...
   */
  function updateTime() {
    const months = (year - 1) * 12 + month;
    time =
      0 <= months ? HijriDate.daysCount(months) : -HijriDate.daysCount(months);
    time += date;
    time *= 864e5;
...
HijriDate.daysInMonth = function (month) {
  return 0 <= month
    ? HijriDate.daysCount(month + 1) - HijriDate.daysCount(month)
    : HijriDate.daysCount(month) - HijriDate.daysCount(month + 1);
};

HijriDate.daysCount = function (month) {
  if (month >= 0) {
    return parseInt(month * HijriDate.moonCycle);
  }
  const times = (parseInt(-month / 30601) + 1) * 30601;
  return (
    parseInt(times * HijriDate.moonCycle) -
    parseInt((times + month) * HijriDate.moonCycle)
  );
};
...
Date.prototype.getDaysInMonth = function () {
  const y = this.getFullYear(),
    isLeapYear = (y % 100 !== 0 && 0 === y % 4) || 0 === y % 400,
    daysInMonth = [
      31,
      isLeapYear ? 29 : 28,
      31,
...
Date.prototype.getMonthName = function (month) {
  return Date.monthNames[
    month === undefined ? this.getMonth() : parseInt(month)
  ];
};

Date.prototype.getMonthShortName = function (month) {
  return Date.monthShortNames[
    month === undefined ? this.getMonth() : parseInt(month)
  ];
};
...
Date.prototype.getDayShortName = function (day) {
  return Date.weekdayShortNames[
    day === undefined ? this.getDay() : parseInt(day)
  ];
};

Date.prototype.getJavaWeekday = function () {
  let day = (this.getTime() - this.getTimezoneOffset() * 6e4) / 864e5;
  day = Math.floor(day);
  day = (day + 3) % 5;
  return 0 > day ? day + 5 : day;
};

Date.prototype.getJavaWeekdayName = function (day) {
  return Date.javaWeekdayNames[
    day === undefined ? this.getJavaWeekday() : parseInt(day)
  ];
};
...
Date.prototype.getFullYearString = function () {
  const y = this.getFullYear();
  return 0 < y
    ? HijriDate.toDigit(y, 4) + " AD"
    : HijriDate.toDigit(Math.abs(y - 1), 4) + " BC";
};
...
Date.prototype.getTimeString = function () {
  return (
    HijriDate.toDigit(this.getHours(), 2) +
    ":" +
