...
function Calendar(
  ...
  var self = this,
    ...
    setShowSettingsAppearance = function () {
      ...
      showSettingsElement.title =
        (isSettingsHidden ? "Show" : "Hide") + " menu settings bar";
      Calendar.markElement(showSettingsElement, !isSettingsHidden);
    },
    setDateModeAppearance = function () {
      changeDateModeElement.title =
        "Change to " + (isHijriMode ? "Gregorian" : "Hijri") + " Calendar";
      Calendar.markElement(changeDateModeElement, isHijriMode);
    },
    setFirstDayOfWeekAppearance = function () {
      changeFirstDayOfWeekElement.title =
        "Set " +
        thisDate.getDayName(firstDayOfWeek ^ 1) +
        " as first day of week";
      Calendar.markElement(changeFirstDayOfWeekElement, !!firstDayOfWeek);
    },
    setAutoHideAppearance = function () {
      changeAutoHideElement.title =
        (isAutoHide ? "Disable" : "Enable") +
        " auto hide calendar on select date";
    ...
    setAutoSelectedDateAppearance = function () {
      changeAutoSelectedDateElement.title =
        (isAutoSelectedDate ? "Disable" : "Enable") +
        " auto selected date on change at year or month values";
      Calendar.markElement(changeAutoSelectedDateElement, isAutoSelectedDate);
    },
    setExtraButtonsVisibilityAppearance = function () {
      changeExtraButtonsVisibilityElement.title =
        (isExtraButtonsVisible ? "Hide" : "Show") +
        " extra buttons to adjust year value";
      Calendar.markElement(
        changeExtraButtonsVisibilityElement,
    ...
    createCalendar = function () {
      ...
      addEvent(
        changeAutoSelectedDateElement,
        "click",
        onChangeAutoSelectedDate
      );
      addEvent(
        changeExtraButtonsVisibilityElement,
        "click",
        onChangeExtraButtonsVisibility
      ),
    ...
    changeCalendarMode = function () {
      for (var i = 0; 12 > i; i++) {
        monthElement.options[i].text = thisDate.getMonthName(i);
      }
      recreateWeekdayRow();
    },
    ...
    createWeekdayRow = function () {
      for (var i = 0; 7 > i; i++) {
        const day = document.createElement("div"),
          ...
          cl =
            5 === wd ? "friday-date" : 0 === wd ? "sunday-date" : "normal-date";
    ...
    createDateGrid = function () {
      ...
      const cdim = thisDate.getDaysInMonth();
      thisDate.setMonth(thisDate.getMonth() - 1);
      const pdim = thisDate.getDaysInMonth();
      thisDate.setMonth(thisDate.getMonth() + 1);
      let fdim = thisDate.getDay(),
      ...
      for (var i = 1; i <= cdim; i++) {
        ...
        if (wd === 5) {
          de.classList.add("friday-date");
        }
        if (wd === 0) {
          de.classList.add("sunday-date");
        }
        if (
        ...
        if (265860e5 <= sd && 266724e5 > sd) {
          de.classList.add("specialz-date");
        }
    ...
    scrollToFix = function () {
      const dw = document.body.offsetWidth,
        ...
        hsSpc = dw > vw ? 20 : 0,
        scrollX = 0 > rect.left ? rect.left : 0,
        scrollY =
    ...
    updateCalendar = function () {
      if (isAutoSelectedDate) {
        const dim = thisDate.getDaysInMonth(),
    ...
    onChangeDate = function (evt) {
      ...
      const prevElm =
        dateGridElement.getElementsByClassName("selected-date")[0];
      if (target !== prevElm) {
        if (!!prevElm) {
  ...
  this.changeDateMode = function () {
    ...
    currentDate = thisDate.getDate();
    thisDate.setTime(selectedDate.getTime());
    selectedDate = isHijriMode
      ? new HijriDate(thisDate.getTime())
